        ...
        try:
            for uid, nn, txt, level, bnn, bl in chatmsg.findall(data):
                ...
                print("[{}({})][lv.{}][{}]: {} ".format(bnn.decode(), bl.decode(), level.decode(),
                                                        nn.decode(errors='ignore'),
                                                        txt.decode(errors='ignore').strip(), ))
                lines = uid.decode() + '->' + nn.decode(errors='ignore') + '->' + level.decode() + '->' + txt.decode(
                    errors='ignore').strip() + '->' + bnn.decode() + '->' + bl.decode()
...
if __name__ == '__main__':
    ...
    while True:
        ...
        p1.start()
        p1.join()  # p1.join()  # print("p1:",p1.is_alive())  # p3.join()  # p2.join()